export const useReferee = () => {
  const getAllReferees = async () => {
    const referees = await $fetch("http://localhost:8000/api/referees");
    return referees;
  };

  const getReferee = async (refereeId: number) => {
    const referee = await $fetch(
      `http://localhost:8000/api/referees/${refereeId}`,
    );
    return referee;
  };

  const createReferee = async (refereeData: object) => {
    const res = await $fetch(`http://localhost:8000/api/referees/create`, {
      method: "POST",
      body: refereeData,
    });
    return res;
  ...
  const deleteReferee = async (refereeId: number) => {
    await $fetch(`http://localhost:8000/api/referees/${refereeId}`, {
      method: "DELETE",
    });
  };
